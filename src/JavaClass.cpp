...
void getContantTagName(cp_info cp, std::string &name)
{
	switch (cp.tag) {
	case CONSTANT_Class:
			name = "Class";
...
{ 
	FILE* f;
	fopen_s(&f, lpszFilePath.c_str(), "rb");
	if (f == NULL)
		return false;
	...
	long fsize = ftell(f);
	m_nByteCodeLength = fsize;
	fseek(f, 0, SEEK_SET);

	u1* string = (u1*)malloc(fsize + 1);
...
BOOL JavaClass::GetConstantPool(u2 nIndex, cp_info& const_pool)
{
	...
	if (CONSTANT_String == const_pool.tag)
	{
		const_pool.info = new u1[2];
		memcpy(const_pool.info, cpool, 2);
	}
...
BOOL JavaClass::GetStringFromConstPool(int nIndex, std::string& strValue)
{
	...
	if((constant_pool[nIndex]->tag != CONSTANT_Utf8) & (constant_pool[nIndex]->tag != CONSTANT_String))
		return FALSE;

	u1 *p =(u1 *)constant_pool[nIndex];

	// if its a string constant the lenght variable contains the index of where the actuin utf8 data is locatied
	// else contains the lenght(bytes) of the utf8
	short length=getu2(&p[1]);

	
	if (constant_pool[nIndex]->tag == CONSTANT_String) {  
		this->GetStringFromConstPool(length, strValue); 
		return TRUE;
	}
	...
	//Note: creating the string in the heap, string issuses - must be fixed
	strValue = *(new std::string(buffer));
	//printf("GetStringFromConstPool: %s\n", buffer);
	delete buffer;
	buffer = 0;
	return TRUE;
}
...
BOOL JavaClass::CreateObject(u2 index, ObjectHeap *pObjectHeap, Object& object)
{
	...
	//DbgPrint(("Creating new object of class [%s]\n"), strClassName);
